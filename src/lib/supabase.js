...
export async function getFolders() {
  const { data, error } = await supabase
    .from('idea_folders')
    .select('*, documents:idea_documents(count)')
    .order('created_at', { ascending: false })
  if (error && error.code !== 'PGRST116') throw error
  
  // Format to include document count
  return (data || []).map(f => ({
    ...f,
    document_count: f.documents?.[0]?.count || 0
  }))
}

export async function createFolder(name, createdBy) {
  const { data, error } = await supabase
    .from('idea_folders')
...
export async function createDocument(folderId, title, content, createdBy) {
  const { data, error } = await supabase
    .from('idea_documents')
    .insert({ folder_id: folderId, title, content, created_by: createdBy })
    .select()
    .single()
  if (error) throw error
  return data
